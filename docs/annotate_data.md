# `annotate_data_updated.py`

User guide for interactive annotation of raw or preprocessed FIF files, with optional automatic break detection

## What this program does

`annotate_data_updated.py` opens an MNE browser on a FIF file so a reviewer can:
...
- save the reviewed annotation state to a sidecar TSV without modifying the original FIF file.

This utility is intended for two common places in a processing pipeline:

- before preprocessing, to mark trim regions at the beginning or end of a run, mark breaks that should be excluded from ICA or automated artifact rejection, or tag obvious bad spans;
...
This version includes all of the earlier annotation-preservation behavior and adds integrated break detection.
...
- the final sidecar is always written, even if the final annotation set is empty.
...
- `--no-browser` lets you update or generate the sidecar without opening the MNE browser.

This makes the tool usable both as an interactive reviewer and as a pipeline utility.

## Core idea

The program manages two different kinds of information that may live in the same TSV sidecar:

- **event rows**, such as stimulus events already present in a BIDS-style `_events.tsv`;
- **annotation rows**, such as `BAD_break`, `BAD_motion`, `spike`, or reviewer-defined artifact spans.

When the sidecar already contains real event rows, the program preserves them and only updates the annotation rows.

That means you can safely use the tool on files that already have experimental events on disk.
...
### 5. Write the sidecar TSV

When the session ends, the program writes the reviewed annotation state to the sidecar TSV.

If the sidecar already contained true event rows, those rows are preserved and the annotation rows are updated.

If the final annotation set is empty, the program still writes the sidecar so that stale annotations are not left behind.

## Sidecar naming rules

You can set the sidecar path explicitly with:

    --annotation-file /path/to/your_sidecar.tsv

If you do not specify `--annotation-file`, the program chooses the path automatically.
...
then the default sidecar is the matching BIDS-style `*_events.tsv`.

Example:

- input: `sub-01_task-rest_meg.fif`
- default sidecar: `sub-01_task-rest_events.tsv`

### Generic FIF names

If the input does not look like a BIDS raw file, the default sidecar is:
...
- default sidecar: `subject01_rest_preproc_annotations.tsv`

### Alternate sidecars

If a default sidecar exists, it is used for the session.

If another plausible sidecar also exists beside it, that alternate file is reported but not written during the session.
...
When a sidecar exists, the program separates its rows into two groups.
...
Example:
    ...
    python annotate_data_updated.py my_run_preproc.fif --clear-current-annotations

If you combine it with automatic break detection:

    python annotate_data_updated.py my_run_preproc.fif \
        --clear-current-annotations \
        --auto-detect-breaks
...
## Output sidecar format

The final sidecar always contains annotation rows written with these columns:
...
### Meaning of the annotation columns

`onset`
: onset time in seconds, relative to the beginning of the loaded FIF file

`duration`
: duration in seconds

`description`
: the MNE annotation label

`trial_type`
: a duplicate of `description`, included for compatibility with event-table workflows

### Empty annotation result

If no annotation rows remain at the end of the session, the program still writes the sidecar.
...
When the program loads an existing sidecar, annotation rows can come from either:
...
### 7. Use a pipeline-specific sidecar path
    ...
    python annotate_data_updated.py my_run.fif \
...
Clicking channel names in the browser still works as before.

Channels marked that way are stored in the browser session’s `bads` list and printed to the terminal as a YAML-ready block when the window closes.
...
Important: this utility still does **not** write bad channels back into the FIF file.

The original design is preserved:

- annotation rows go to the sidecar TSV;
- bad channels are reported for manual insertion into your pipeline configuration.
...
`--annotation-file`
: explicit path to the sidecar TSV
...
Because the tool does not modify the original FIF file.

Bad channels are printed for manual transfer into your pipeline configuration.
...
`annotate_data_updated.py` is now both an interactive annotation tool and a practical pipeline utility. It preserves existing annotations by default, can start from a blank annotation set when requested, can preserve event rows already present in an `_events.tsv`, can auto-detect and mark long breaks between task periods, and keeps the original FIF file untouched throughout the review process.
